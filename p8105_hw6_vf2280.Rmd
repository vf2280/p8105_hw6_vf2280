...
`fit_0` model shows high average RMSE compared to the `fit_1` and `full` model. The `full` model shows to have the lowest average RMSE making it one of the better models of the three. However, this model is more complex and perhaps, harder to interpret. So, the `fit_1` model may be better in terms of fit and interpretation.
...
As for `log_b0_1`, the distribution shows heavy tails as well with a center around roughly 1.95. This shows that the log-normal interaction indicates that for every 1 degree increase in tmin, there is a 1.95 increase in tmax.
